...
class Camera(Node):
    ...
    def __init__(self, framerate, queue):
        ...
        self.__stream = self.create_publisher(Image, 'camera_stream', queue)
        self.__timer = self.create_timer(1/framerate, self.capture)
        self.__camera = cv2.VideoCapture(4)
        #self.camera.set(cv2.CAP_PROP_FRAME_WIDTH, 1080)
        #self.camera.set(cv2.CAP_PROP_FRAME_WIDTH, 1920)
        cv2.imshow('camera', self.__camera.read()[1])
        cv2.waitKey(2000)
