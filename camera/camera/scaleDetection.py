...
class ScaleDetector:
    ...
    def __init__(self, delta1, delta2, n, phi1, phi2, m, count, accuracy):
    ...
    def calculateLineEnds(shape, anchors, vectors):
        ...
        if 0 < i.shape[0]:
            s = (anchors[i,1]*vectors[i,0]+(shape[1]-anchors[i,0])*vectors[i,1])/vectors[i,0]
        ...
        if 0 < i.shape[0]:
            s = (anchors[i,0]*vectors[i,1]-(anchors[i,1]-shape[0])*vectors[i,0])/vectors[i,1]
            i = i[np.logical_and(0<s, s<=shape[1])]
            if 0 < i.shape[0]:
                ends[i,j[i],0] = s[i].astype(np.uint16)
    ...
    def scanArea(self, center):
        ...
        ends = ScaleDetector.calculateLineEnds(shape=(self.__img.shape[0]-1,self.__img.shape[1]-1), anchors=anchors, vectors=np.tile(lineVectors, (2*self.__n+1,1)))
        line = None
        for e in ends:
    ...
    def findCorners(self, left, right):
        ...
        for i in range(4):
            ...
            while phi < pi/2:
                ...
                ends = ScaleDetector.calculateLineEnds(shape=(self.__img.shape[0]-1,self.__img.shape[1]-1), anchors=np.array([left], dtype=np.uint16) if i<2 else np.array([right], dtype=np.uint16), vectors=vector)[0]
    ...
    def detectScale(self, img):
        ...
        while self.__nextLine():
            ...
            ends = ScaleDetector.calculateLineEnds(shape=(img.shape[0]-1,img.shape[1]-1), anchors=np.array([anchor]), vectors=np.array([[sin(self.__alpha), -cos(self.__alpha)]], dtype=np.float32))[0]
